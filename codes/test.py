...
if __name__ ==  '__main__':  
    ...
    print('Combined Evaluation of Folds [m,n]...') 
    if labeled_Data: 
        all_Missed_c = list()
        for fold_idx in tqdm(range(loop_start, loop_end), desc="Combined Evaluation"):
            ...
            cm_plot.savefig(save_path + f'/{model_name}_Confusion_Matrix_fold_{fold_idx}.png', dpi=600)
            # Generate Multiclass ROC curve figure
            roc_plot = plot_multiclass_roc(targets, pred_probs, categories)
            roc_plot.savefig(save_path + f'/{model_name}_ROC_plot_fold_{fold_idx}.png', dpi=600)
            ...
            prc_plot.savefig(save_path + f'/{model_name}_PRC_plot_fold_{fold_idx}.png', dpi=600)
            Eval_Mat = []
            # Per class metricies
            ...
            # Save to excel file   
            new_savepath = save_path + f'/{model_name}_fold_{fold_idx}.xlsx'  # file to save 
        ...
        for fold_idx in tqdm(range(loop_start, loop_end), desc="Combined Evaluation"):
            ...
            # save to excel file   
            new_savepath = Results_path +'/'+  model_name + f'{model_name}.xlsx'  # file to save 
            writer = pd.ExcelWriter(new_savepath, engine='openpyxl')
