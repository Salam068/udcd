...
if __name__ ==  '__main__':
    ...
    isDual = config['train_dual'] # set to 'True'
    ...
    optim_scheduler = 'ReduceLROnPlateau'

    print(optim_scheduler)
    ...
    for fold_idx in range(loop_start, loop_end):
        ...
        val_ds = MyData(root_dir=valdir_fold, categories=categories, img_names=img_names_val, target=labels_val, my_transforms=my_test_transforms, return_path=False, ONN=ONN, mean=input_mean, std=input_std) # type: ignore
        val_dl = DataLoader(val_ds, batch_size=batch_size, shuffle=False, pin_memory=True, num_workers=1)
        # Test Dataloader
        test_ds = MyData(root_dir=testdir_fold, categories=categories, img_names=img_names_test, target=labels_test, my_transforms=my_test_transforms, return_path=True, ONN=ONN, mean=input_mean, std=input_std) # type: ignore
        ...
        if load_weights: 
            try:
                ...
                model = get_pretrained_model(parentdir, model_to_load, model_mode, isPretrained, input_ch, class_num, final_activation_func, train_on_gpu, multi_gpu, q_order)  # type: ignore
                ...
                if encoder:
                    model = EncoderModel(model.encoder)  # type: ignore # ResNet
                    model = model.to('cuda')
            except:
