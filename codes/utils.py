...
class MyData(Dataset):
    ...
    def __getitem__(self, index):
        ...
        if self.return_path: 
            ...
            print(f"Returning (x, y, path): {x.shape}, {y}, {self.categories[label]+'/'+self.img_names[index]}")
            return x, y,  self.categories[label]+'/'+self.img_names[index] 
        else:
            print(f"Returning (x, y): {x.shape}, {y}")
            return x, y
...
def train(model_to_load, 
    ...
    total_time = timer() - overall_start
    print('==============================================================')
    print(f'Best epoch: {best_epoch} with loss: {valid_loss_min:.2f} and acc: {100 * valid_best_acc:.2f}%')
