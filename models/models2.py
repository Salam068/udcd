...
#     def __init__(self, in_channels, class_num, final_activation=None):
...
#         return x
# def get_model(model_name, class_num, final_activation_func, is_pretrained=True):
#     # Determine pretrained weights
#     pretrained_weights = 'DEFAULT' if is_pretrained else None
#
#     # Factory for accessing models
#     model_factory = {
...
#
#     model = model_factory[model_name](weights=pretrained_weights)
#
#     # Replace the classifier based on architecture
#     if hasattr(model, 'fc'):  # For ResNet, RegNet, etc.
...
#         model.classifier[-1] = CNN_Classifier(in_channels=num_ftrs, class_num=class_num,
#                                               final_activation=final_activation_func)
#     elif model_name.startswith('squeezenet'):  # Special handling for SqueezeNet
#         model.classifier[-1] = nn.Sequential(
#             nn.AdaptiveAvgPool2d(output_size=(1, 1)),
#             nn.Flatten(),
#             CNN_Classifier(in_channels=1000, class_num=class_num, final_activation=final_activation_func)
#         )
#     elif model_name == 'inception_v3':  # Special handling for Inception
#         num_ftrs = model.AuxLogits.fc.in_features
#         model.AuxLogits.fc = CNN_Classifier(in_channels=num_ftrs, class_num=class_num,
#                                             final_activation=final_activation_func)
#         num_ftrs = model.fc.in_features
#         model.fc = CNN_Classifier(in_channels=num_ftrs, class_num=class_num, final_activation=final_activation_func)
#     else:
#         raise NotImplementedError(f"Model architecture for {model_name} requires special handling.")
#
#     return model
...
# SelfONN Blocks
from codes.selfonn import SelfONNLayer
...
class Self_B_ResBlock(nn.Module):
    def __init__(self, in_channels=3, channel1=8, channel2=16, channel3=8, resConnection=False, q_order=3):
        super().__init__()
        self.layer1 = SelfONNLayer(in_channels=in_channels, out_channels=channel1, kernel_size=1, stride=1, padding=0,
                                   dilation=1, groups=1, bias=True, q=q_order, mode='fast')
        ...
        if self.resConnection:
            self.layer2 = SelfONNLayer(in_channels=channel1, out_channels=channel2, kernel_size=3, stride=1, padding=1,
                                       dilation=1, groups=1, bias=True, q=q_order, mode='fast')
            self.Batch2 = nn.BatchNorm2d(channel2, eps=1e-05, momentum=0.1, affine=True, track_running_stats=True)
        else:
            self.layer2 = SelfONNLayer(in_channels=channel1, out_channels=channel2, kernel_size=3, stride=2, padding=1,
                                       dilation=1, groups=1, bias=True, q=q_order, mode='fast')
            self.Batch2 = nn.BatchNorm2d(channel2, eps=1e-05, momentum=0.1, affine=True, track_running_stats=True)
        self.layer3 = SelfONNLayer(in_channels=channel2, out_channels=channel3, kernel_size=1, stride=1, padding=0,
                                   dilation=1, groups=1, bias=True, q=q_order, mode='fast')
...
class Self_MobileNet(nn.Module):
    def __init__(self, input_channel=3, last_layer_channel=32, class_num=10):
        ...
        self.selfONN = SelfONNLayer(in_channels=input_channel, out_channels=32, kernel_size=3, stride=2, padding=1,
                                    dilation=1, groups=1, bias=True, q=3, mode='fast')
        self.batchnorm = nn.BatchNorm2d(32, eps=1e-05, momentum=0.1, affine=True, track_running_stats=True)
        self.tanh = nn.Tanh()
        self.BottleneckResidual1 = Self_B_ResBlock(in_channels=32, channel1=16, channel2=48, channel3=32,
                                                   resConnection=True)
        self.NonResidual1 = Self_B_ResBlock(in_channels=32, channel1=32, channel2=48, channel3=16, resConnection=False)
        self.BottleneckResidual2 = Self_B_ResBlock(in_channels=16, channel1=24, channel2=48, channel3=16,
                                                   resConnection=True)
        self.NonResidual2 = Self_B_ResBlock(in_channels=16, channel1=24, channel2=32, channel3=48, resConnection=False)
        self.BottleneckResidual3 = Self_B_ResBlock(in_channels=48, channel1=16, channel2=56, channel3=48,
                                                   resConnection=True)
        self.NonResidual3 = Self_B_ResBlock(in_channels=48, channel1=8, channel2=48, channel3=36, resConnection=False)
        self.BottleneckResidual4 = Self_B_ResBlock(in_channels=36, channel1=16, channel2=32, channel3=36,
                                                   resConnection=True)
        self.BottleneckResidual5 = Self_B_ResBlock(in_channels=36, channel1=8, channel2=32, channel3=36,
                                                   resConnection=True)
        self.NonResidual4 = Self_B_ResBlock(in_channels=36, channel1=8, channel2=32, channel3=last_layer_channel,
                                            resConnection=False)
...
class Self_DenseMobileNet(nn.Module):
    def __init__(self, input_channel=3, last_layer_channel=32, class_num=10, q_order=3):
        ...
        self.batchnorm = nn.BatchNorm2d(32, eps=1e-05, momentum=0.1, affine=True, track_running_stats=True)
        self.tanh = nn.Tanh()
        self.BottleneckResidual1 = Self_B_ResBlock(in_channels=32, channel1=64, channel2=64, channel3=32,
                                                   resConnection=True, q_order=q_order)
        self.BottleneckResidual2 = Self_B_ResBlock(in_channels=32, channel1=72, channel2=72, channel3=32,
                                                   resConnection=True, q_order=q_order)
        self.maxpool = nn.MaxPool2d(2)
        self.BottleneckResidual3 = Self_B_ResBlock(in_channels=32, channel1=84, channel2=84, channel3=32,
                                                   resConnection=True, q_order=q_order)
        self.BottleneckResidual4 = Self_B_ResBlock(in_channels=32, channel1=96, channel2=96, channel3=32,
                                                   resConnection=True, q_order=q_order)
        self.BottleneckResidual5 = Self_B_ResBlock(in_channels=32, channel1=96, channel2=96, channel3=32,
                                                   resConnection=True, q_order=q_order)
        self.BottleneckResidual6 = Self_B_ResBlock(in_channels=32, channel1=84, channel2=84, channel3=32,
                                                   resConnection=True, q_order=q_order)
        self.BottleneckResidual7 = Self_B_ResBlock(in_channels=32, channel1=72, channel2=72, channel3=32,
                                                   resConnection=True, q_order=q_order)
        self.BottleneckResidual8 = Self_B_ResBlock(in_channels=32, channel1=64, channel2=64, channel3=32,
                                                   resConnection=True, q_order=q_order)
...
def SelfONN_1(input_ch, class_num, q_order):
    model = torch.nn.Sequential(
        ...
        # 1st layer (conv)
        SelfONNLayer(in_channels=input_ch, out_channels=75, kernel_size=3, stride=1, padding=1, dilation=1, groups=1,
                     bias=True, q=q_order, mode='fast'),
        torch.nn.BatchNorm2d(75, eps=1e-05, momentum=0.1, affine=True, track_running_stats=True),
        torch.nn.MaxPool2d(2),
        torch.nn.Tanh(),
        # 2nd layer (conv)
        SelfONNLayer(in_channels=75, out_channels=56, kernel_size=3, stride=1, padding=1, dilation=1, groups=1,
                     bias=True, q=q_order, mode='fast'),
...
def SelfONN_2(input_ch, class_num, q_order):
    model = torch.nn.Sequential(
        # 1st layer (conv)
        SelfONNLayer(in_channels=input_ch, out_channels=8, kernel_size=3, stride=1, padding=1, dilation=1, groups=1,
                     bias=True, q=q_order, mode='fast'),
        torch.nn.MaxPool2d(2),
        torch.nn.Tanh(),
        # 2nd layer (conv)
        SelfONNLayer(in_channels=8, out_channels=8, kernel_size=3, stride=1, padding=1, dilation=1, groups=1, bias=True,
                     q=q_order, mode='fast'),
        torch.nn.MaxPool2d(2),
        torch.nn.Tanh(),
        # 3rd layer (conv)
        SelfONNLayer(in_channels=8, out_channels=16, kernel_size=3, stride=1, padding=1, dilation=1, groups=1,
                     bias=True, q=q_order, mode='fast'),
        torch.nn.MaxPool2d(2),
        torch.nn.Tanh(),
        # 4th layer (conv)
        SelfONNLayer(in_channels=16, out_channels=16, kernel_size=3, stride=1, padding=1, dilation=1, groups=1,
                     bias=True, q=q_order, mode='fast'),
...
def SelfONN_2_Dense(input_ch, class_num, q_order):
    model = torch.nn.Sequential(
        ...
        SelfONNLayer(in_channels=16, out_channels=class_num, kernel_size=3, stride=1, padding=0, dilation=1, groups=1,
                     bias=True, q=q_order, mode='fast'),
        SqueezeLayer(),
        torch.nn.LogSigmoid()
    )
...
def SelfONN_3(input_ch, class_num, q_order):
    model = torch.nn.Sequential(
        # 1st layer (conv)
        SelfONNLayer(in_channels=input_ch, out_channels=8, kernel_size=3, stride=1, padding=1, dilation=1, groups=1,
                     bias=True, q=q_order, mode='fast', dropout=0.2),
        torch.nn.MaxPool2d(2),
        torch.nn.Tanh(),
        # 2nd layer (conv)
        SelfONNLayer(in_channels=8, out_channels=8, kernel_size=3, stride=1, padding=1, dilation=1, groups=1, bias=True,
        ...
        # 4th layer (conv)
        SelfONNLayer(in_channels=8, out_channels=8, kernel_size=3, stride=1, padding=1, dilation=1, groups=1, bias=True,
                     q=q_order, mode='fast', dropout=0.2),
        torch.nn.MaxPool2d(2),
        torch.nn.Tanh(),
        # 5th layer (conv)
        SelfONNLayer(in_channels=8, out_channels=32, kernel_size=3, stride=1, padding=1, dilation=1, groups=1,
                     bias=True, q=q_order, mode='fast'),
        # torch.nn.MaxPool2d(2),
        torch.nn.Tanh(),
        # 6th layer (conv)
        SelfONNLayer(in_channels=32, out_channels=32, kernel_size=3, stride=1, padding=1, dilation=1, groups=1,
                     bias=True, q=q_order, mode='fast'),
        torch.nn.MaxPool2d(2),
        torch.nn.Tanh(),
        # flatten
...
def SelfONN_4(input_ch, class_num, q_order):
    model = torch.nn.Sequential(
        ...
        torch.nn.Tanh(),
        # 5th layer (conv)
        SelfONNLayer(in_channels=16, out_channels=32, kernel_size=3, stride=1, padding=1, dilation=1, groups=1,
                     bias=True, q=q_order, mode='fast'),
        torch.nn.MaxPool2d(2),
        torch.nn.Tanh(),
        # 6th layer (conv)
...
def get_pretrained_model(model_name, model_mode="import_Torch", isPretrained=True, input_ch=3, class_num=10, final_activation_func='Softmax',
    ...
    if model_mode == "custom_CNN":
        if model_name == 'cnn_V1':
            ...
            raise ValueError(
                'The requested Custom CNN model is not currently available or there is a typo. Please also check the configs.')
        ...
        if model_name == 'SelfONN_1':
            ...
            model = Self_DenseMobileNet(input_channel=input_ch, last_layer_channel=32, class_num=class_num,
                                        q_order=q_order)

        else:
            raise ValueError(
                'The requested Custom Self-ONN model is not currently available or there is a typo. Please also check the configs.')
        ...
        if model_name == 'alexnet':
            ...
            model.classifier = CNN_Classifier(in_channels=num_ftrs, class_num=class_num,
                                              final_activation=final_activation_func)
            ...
            model.head = CNN_Classifier(in_channels=num_ftrs, class_num=class_num,
                                        final_activation=final_activation_func)
            ...
            raise ValueError(
                'The requested Pretrained CNN model is not currently available in the PyTorch repository or there is a typo. Please also check the configs.')
        ...
        raise ValueError(
            'The requested MODE is not currently available in the pipeline or there is a typo. Please also check the configs.')

        # Move to gpu and parallelize
    device = 'cuda' if torch.cuda.is_available() else 'cpu'
    print(f"{model_name} is loaded to train on {device}")
    if train_on_gpu:
...
class BaseModelWithFeatures(nn.Module):
    def __init__(self, model_name, hidden_units, out_size, drop_rate=0, pretrained=True):
        super(BaseModelWithFeatures, self).__init__()
        ...
        self.base_model = getattr(models, model_name)(weights=pretrained_weights)
        # print(self.base_model)

        print(f"Hidden units: {hidden_units} and outsize: {out_size}")

        if hidden_units is None or out_size is None:
            raise ValueError("Both hidden_units and out_size must be provided and cannot be None.")

        # Get number of features based on model architecture
        if hasattr(self.base_model, 'classifier'):
            if isinstance(self.base_model.classifier, nn.Sequential):
                num_ftrs = self.base_model.classifier[-1].in_features  # For VGG, AlexNet
            else:
                num_ftrs = self.base_model.classifier.in_features  # For DenseNet
        elif hasattr(self.base_model, 'fc'):
            num_ftrs = self.base_model.fc.in_features  # For ResNet, RegNet
        elif hasattr(self.base_model, 'AuxLogits'):
            num_ftrs = self.base_model.AuxLogits.fc.in_features  # For Inception

        # Create new layers
        self.fc_layer = nn.Linear(num_ftrs, hidden_units)
        self.classifier = nn.Linear(hidden_units, out_size)
        ...
        if hasattr(self.base_model, 'classifier'):
            ...
            self.base_model.AuxLogits.fc = nn.Identity()
    ...
    def forward(self, x):
        ...
        if hasattr(self.base_model, 'features'):
            ...
            features = self.base_model(x)  # For models without explicit feature extractor
        ...
        feature4 = self.fc_layer(features)

        # Get final output
        logit_b4 = self.classifier(feature4)
